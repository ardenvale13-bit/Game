...
export type GameType = 'pictionary' | 'cah' | 'codenames' | 'wmlt' | 'wyr' | 'hangman' | 'guess-betrayal' | 'meme' | 'familyfeud' | 'guesswho' | 'uno' | null;
...
const useLobbyStore = create<LobbyState & LobbyActions>((set, get) => ({
  ...
  canStartGame: () => {
    ...
    if (state.selectedGame === 'guesswho') return state.players.length >= 3;
